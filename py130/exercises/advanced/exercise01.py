...
# marks (?). You should treat any sequence of characters that are not spaces
# or sentence-ending characters as a word. Thus, -- should count as a word.
# Log the longest sentence and its word count. Pay attention to the expected
# output, and be sure you preserve the punctuation from the end of the
# sentence.

# Note that this problem is about manipulating and processing strings. As
# such, every detail about the string matters (e.g., case, punctuation, tabs,
# spaces, etc.).
...
def longest_sentence(text):
    ...
    sentence_counts = {sentence: len(sentence.split()) for sentence in sentences}
    max_sentence = max(sentence_counts, key=sentence_counts.get)
    max_count = sentence_counts[max_sentence]
    print(max_sentence)
    print()
