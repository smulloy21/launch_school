# As we saw in the previous exercises, a matrix can be represented by a list
# of lists. For example, the 3x3 matrix shown below:
...
def rotate90(matrix):
    ...
    return [[matrix[j][i] for j in range(rows - 1, -1, -1)] for i in range(columns)]
